...
def send_welcome(message):
    ...
    if len(cur.fetchall()) == 0:
        bot.send_message(message.from_user.id, "Начата регистрация, введи своё имя. Формат: Фамилия и первая буква имени, с заглавных букв")
        ...
        if cur.fetchall()[0][0] == 0:
            ...
            bot.send_message(message.from_user.id,
                             f"\u2705 Ты <b>записан</b> в очередь\n<b>Сейчас твой номер - {cur.fetchall()[0][0]}</b>\nБлиже к занятию ты получишь полный список всех записанных\nУдалить запись можно прямо сейчас",
                             reply_markup=markup)
...
def edit_user_status_3(message):
    ...
    if message.text == 'Да, удалить запись':
        ...
        cur_queue = cur.fetchall()[0][0] + 1
        cur.execute('SELECT tg_id FROM users WHERE editing = 1')
        bot.send_message(cur.fetchall()[0][0],
                         f"\u26A0 Ты был записан в очередь администратором. Сейчас твой номер - {cur_queue}")
...
def start_collect_feedback_3(message):
    ...
    if not now_fb:
        ...
        btn1 = types.InlineKeyboardButton("Да, я выступил", callback_data='fb_yes')
        btn2 = types.InlineKeyboardButton("Нет, я НЕ выступил", callback_data='fb_no')
        markup.add(btn1, btn2)
